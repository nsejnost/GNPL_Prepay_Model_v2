...
# Issuer name normalization. GNMA disclosure uses the legal entity name in
# effect at each disclosure period, but corporate rebranding can cause the
# SAME entity to appear under multiple names across periods. The most
# material case is ORIX Real Estate Capital, which was rebranded to Lument
# Real Estate Capital in October 2020 (after the merger of ORIX, Hunt Real
# Estate Capital, and RED Capital Group). GNMA disclosures continued
# using the ORIX name through November 2022, then switched to Lument
# starting in December 2022. Both names share the same issuer_number (3896).
#
# Without normalization, downstream models treat ORIX (2019-2022, high-CPR
# COVID era) and Lument (2023-2026, post-COVID era) as two different
# issuers and confound issuer-specific behavior with rate-environment
# effects. Normalizing both to a single canonical name lets time-period
# features (e.g. is_post_covid) properly capture the era effect while
# issuer features capture the entity-level behavior.
ISSUER_NAME_NORMALIZATION = {
    # Map legacy names → current canonical name (uppercase comparison)
    'ORIX REAL ESTATE CAPITAL, LLC': 'LUMENT REAL ESTATE CAPITAL,LLC',
    'ORIX REAL ESTATE CAPITAL, LLC (LP)': 'LUMENT REAL ESTATE CAPITAL,LLC',
}
def normalize_issuer_name(name):
    """Apply known corporate-rebrand mappings to the issuer_name field
    so the same legal entity is consistently identified across periods.
    Pass-through for any name not in the mapping."""
    if not name: return name
    return ISSUER_NAME_NORMALIZATION.get(str(name).strip().upper(), name)
...
def read_mfplmon3(filepath):
    ...
    for line in text.split('\n'):
        ...
        rec = {
            ...
            'issuer_number': g(PX['issuer_number']),
            'issuer_name': normalize_issuer_name(g(PX['issuer_name'])),
...
# The fha_program_code field arrives as a free-form string like
# "221(d)(4)", "223(f)", "232/223(f)", "538", "232", "223(a)(7)".
# Combinations like "232/223(f)" mean a 232-insured healthcare property
# being refinanced under the 223(f) acquisition program — that is a
# refinance, not new construction, so loan_purpose='RP' even though the
# property is healthcare (is_hc_232=1).
#
# The dummy flags (is_*) are NOT mutually exclusive: "232/223(f)" lights
# up both is_hc_232=1 and is_223f=1 by design, because downstream models
# may want either signal. The fha_category single-pick field is provided
# for cases where one categorical is needed.

def classify_fha_dummies(code):
    """Return dict of independent FHA program flags from a code string.

    Returns flags for healthcare (232), 221(d)(4), 223(a)(7), 223(f),
    538, 220, and 241. These are NOT mutually exclusive — e.g.
    '232/223(f)' sets both is_hc_232 and is_223f.
    """
    c = (code or '').strip().upper().replace(' ', '')
    return {
        'is_hc_232':  1 if '232' in c else 0,
        'is_221d4':   1 if ('221(D)(4)' in c or '221D4' in c) else 0,
        'is_223a7':   1 if ('223(A)(7)' in c or '223A7' in c) else 0,
        'is_223f':    1 if ('223(F)' in c  or '223F'  in c) else 0,
        'is_538':     1 if '538'  in c else 0,
        'is_220':     1 if ('220' in c and '221' not in c.replace('220', '', 1)) else 0,
        'is_241':     1 if '241'  in c else 0,
    }


def classify_fha_category(code):
    """Return a single categorical FHA bucket (one-hot pick).

    Priority order: 538 > 232 > 221d4 > 223a7 > 223f > 220 > 241 > OTHER.
    The 232 priority means '232/223(f)' lands in '232' (healthcare
    dominates for the categorical view), while loan_purpose still
    correctly reads it as 'RP'.
    """
    c = (code or '').strip().upper().replace(' ', '')
    if not c:
        return 'OTHER'
    ...
    if '232' in c:
        return '232'
    if '221(D)(4)' in c or '221D4' in c:
        return '221d4'
    if '223(A)(7)' in c or '223A7' in c:
        return '223a7'
    if '223(F)' in c or '223F' in c:
        return '223f'
    if '220' in c:
        return '220'
    if '241' in c:
...
def classify_loan_purpose(code):
    """Classify loan as 'NC' (new construction), 'RP' (refi/purchase),
    '538' (USDA rural, separate category per BAM), or 'OTHER'.

    The Bloomberg BAM model treats NC vs RP as a primary cut because
    NC loans exhibit pent-up refi demand after construction conversion
    AND higher buyout rates during stabilization. Mapping rules:
      - 538 takes precedence (USDA programs are a distinct sector)
      - Any code containing 223(f) or 223(a)(7) is RP, even when
        combined with 232 (e.g. '232/223(f)' is a healthcare RP)
      - 221(d)(4), 220, 241, and standalone 232 are NC
      - Everything else is OTHER (rare legacy programs)
    """
    c = (code or '').strip().upper().replace(' ', '')
    if not c:
        return 'OTHER'
    if '538' in c:
        return '538'
    # Refi indicators take precedence over 232 (handles 232/223(f) etc.)
    if '223(F)' in c or '223F' in c or '223(A)(7)' in c or '223A7' in c:
        return 'RP'
    # New construction programs
    if ('221(D)(4)' in c or '221D4' in c or '232' in c or
            '220' in c or '241' in c):
        return 'NC'
    return 'OTHER'


def compute_loan_age_months(orig_date, period):
    """Compute loan age in months from origination_date (YYYYMMDD or
    YYYYMM) to the observation period (YYYYMM). Returns NaN if either
    date is unparseable. Negative ages are clamped to 0 (occurs rarely
    when origination_date is misreported as the first-pay date).

    Sanity bounds: GNMA's project-loan program started in the late
    1960s, and origination cannot reasonably occur in the future
    relative to the observation period. Out-of-range values (typos in
    the source disclosure data — e.g. "10021114" instead of "20021114"
    for a single 232/223(f) loan, or "20830500" instead of "20030500"
    for another) would otherwise produce ages of ~1000 years and
    pollute the SATO vintage-median map. Returning NaN keeps these
    handful of corrupted rows out of computations that depend on age.
    """
    od = parse_date_yyyymmdd(orig_date)
    if not od or len(od) < 6 or not period or len(period) < 6:
        return np.nan
    try:
        oy, om = int(od[:4]), int(od[4:6])
        py, pm = int(period[:4]), int(period[4:6])
        # Reject obviously-corrupt origination years. The +1 lenience
        # on the upper bound handles legitimate end-of-year edge
        # cases where origination_date is in the same calendar year
        # as the observation period but a few days/weeks ahead.
        if oy < 1965 or oy > py + 1:
            return np.nan
        if om < 1 or om > 12:
            return np.nan
        age = (py - oy) * 12 + (pm - om)
...
def build_analytics(file_by_period):
    """Parse files and enrich records with prepayment flags, lockout
    status, refi incentive, FHA classification, loan age, vintage, and
    SATO.

    Filtering philosophy: this function does NOT drop rows. Every
    loan-month record in the input is preserved in the output. Filtering
    is expressed via flag columns so downstream model code can apply
    the appropriate filter for its task:

      prepay_eligible       : voluntary refi/cashout modeling
                              (excludes CL/CS, lockout, invalid data)
      involuntary_eligible  : buyout/default modeling
                              (excludes CL/CS, invalid data; lockout
                              loans CAN have buyouts so they're kept)
      has_valid_data        : minimum data quality bar

    CL/CS pools are ALWAYS prepay/buyout = 0 because (a) construction
    pools are locked out by definition and (b) their disappearance
    represents pool conversion to PN, not a prepay event. The "new
    construction" effect Bloomberg models for post-conversion loans is
    captured separately via is_new_construction (FHA-code-based).

    Memory strategy: files are read and converted to DataFrames one
    period at a time. A sliding window keeps at most two periods of raw
    Python dicts alive simultaneously (current + next), so peak memory
    scales with a single period rather than all 88 at once.
    """
    periods = sorted(file_by_period.keys())
    print(f"\n[parse+analytics] Parsing and enriching {len(periods)} periods...")

    # Load PLC rates (already in bps)
    plc_rates = load_plc_rates()

    period_dfs = []
    recs_cache = None   # sliding window: next period pre-read as "current"

    for i, period in enumerate(periods):
        nxt = periods[i + 1] if i + 1 < len(periods) else None

        # ── Read current period ──────────────────────────────────────
        # Re-use recs_cache if this period was pre-read while processing
        # the previous one; otherwise read from disk now.
        if recs_cache is not None:
            recs = recs_cache
            recs_cache = None
        else:
            recs = read_mfplmon3(file_by_period[period]) or []
        ...
        for r in recs:
            pt[r['pool_type']] += 1
        print(f"  {os.path.basename(file_by_period[period])} -> {period}: "
              f"{len(recs)} records {dict(pt)}")

        # ── Pre-read next period for the "disappeared" loan_id check ─
        # Reading the full next period here lets us reuse it as the
        # "current" period in the next loop iteration so each file is
        # read exactly once.
        if nxt:
            recs_cache = read_mfplmon3(file_by_period[nxt]) or []
            nl = {r['loan_id'] for r in recs_cache}
            if not nl:
                # Next-period parse returned nothing (corrupt zip,
                # unrecognized layout, network glitch on a re-download,
                # etc.). Without a valid loan_id set for the next
                # period, we cannot tell which loans actually
                # disappeared. Print a clear warning so this isn't a
                # silent failure mode that injects false-positive
                # prepay events for the entire prior period.
                print(f"  ! warning: next period {nxt} parsed empty; "
                      f"disappearance check disabled for period {period}")
        else:
            recs_cache = None
            nl = set()
        ...
        for r in recs:
            ...
            # Construction-phase flag. CL/CS pools represent loans
            # currently in the construction draw period — they are
            # locked-out by definition, and their "disappearance" from
            # the panel almost always means conversion to PN, not a
            # prepay event.
            is_construction = 1 if pool_type in ('CL', 'CS') else 0
            r['is_construction_phase'] = is_construction

            # Lockout / penalty status from date fields
            ...
            # Construction loans are effectively in lockout regardless of
            # what the lockout_end_date field says (sometimes blank for CL/CS).
            if is_construction:
                in_lockout = 1
            ...
            # Current penalty points from the declining schedule
            ...
            if pd.notna(loan_rate) and pd.notna(plc_bps):
                ...
                r['gross_refi_incentive_bps'] = round(net_coupon_bps - plc_bps, 2)
            else:
                r['plc_rate_bps'] = np.nan
                r['refi_incentive_bps'] = np.nan
                r['gross_refi_incentive_bps'] = np.nan

            # ── Granular delinquency flags ─────────────────────────
            mdq = r.get('months_dq', 0) or 0
            r['is_dq_30'] = 1 if mdq >= 1 else 0
            r['is_dq_60'] = 1 if mdq >= 2 else 0
            r['is_dq_90'] = 1 if mdq >= 3 else 0
            ...
            r.update(classify_fha_dummies(fha))
            r['fha_category'] = classify_fha_category(fha)

            # ── Loan purpose: NC vs RP (Bloomberg's primary cut) ───
            purpose = classify_loan_purpose(fha)
            r['loan_purpose'] = purpose
            r['is_new_construction'] = 1 if purpose == 'NC' else 0
            r['is_refi_purchase']   = 1 if purpose == 'RP' else 0

            # ── Loan age and vintage ───────────────────────────────
            orig_date = r.get('origination_date', '')
            age_m = compute_loan_age_months(orig_date, period)
            r['loan_age_months'] = age_m
            od = parse_date_yyyymmdd(orig_date)
            try:
                # Apply the same sanity bounds as compute_loan_age_months
                # so a single bad vintage doesn't contaminate the SATO
                # vintage-median map. Without this guard, the two
                # corrupt-origination loans (oy=1002, oy=2083) appear
                # as their own one-loan vintages with degenerate
                # medians, breaking SATO for everyone in nearby years.
                py = int(period[:4]) if period and len(period) >= 4 else None
                yr = int(od[:4]) if od and len(od) >= 4 else None
                if yr is None or py is None or yr < 1965 or yr > py + 1:
                    r['vintage_year'] = np.nan
                else:
                    r['vintage_year'] = yr
            except (ValueError, IndexError):
                r['vintage_year'] = np.nan
            ...
            # ── Data-quality flag ──────────────────────────────────
            upb = r.get('upb', np.nan)
            has_valid = (
                pd.notna(loan_rate) and
                pd.notna(upb) and (upb or 0) > 0
            )
            r['has_valid_data'] = 1 if has_valid else 0

            # ── Eligibility flags (filtering moved from row-drop to
            #    flag-based; downstream models filter by these) ─────
            if is_construction or not has_valid:
                # CL/CS rule: 0% prepay/buyout always. Per the GNMA
                # multifamily disclosure pattern, CL/CS loans disappear
                # because their pool converts to a PN pool with a new
                # CUSIP — that's a relabeling, not a prepayment.
                r['prepay_eligible'] = 0
                r['involuntary_eligible'] = 0
            else:
                r['prepay_eligible']      = 1 if not in_lockout else 0
                # Buyouts can occur during lockout (default doesn't care
                # about the prepayment lockout), so don't gate on it.
                r['involuntary_eligible'] = 1

            # ── Prepayment event flags ─────────────────────────────
            lid = r.get('loan_id', '')
            rm = (r.get('removal_reason', '') or '').strip()
            # Gate the disappearance check on whether we actually have
            # a valid next-period loan_id set (`nl`). When `nl` is
            # empty — which can happen if the next period file failed
            # to parse — every loan would otherwise be flagged as
            # disappeared, injecting thousands of false-positive
            # voluntary prepays for the period. Treat "no next-period
            # evidence" as "cannot determine disappearance", which is
            # the conservative and correct choice.
            disappeared = bool(nl) and (lid not in nl)
            r['disappeared_next_period'] = 1 if disappeared else 0

            if is_construction or not has_valid:
                # Construction phase: always 0. Disappearance is pool
                # conversion to PN, captured via exit_type below.
                r['prepaid_voluntary']   = 0
                r['prepaid_involuntary'] = 0
            else:
                # GNMA disclosure L28 Removal Reason codes (per V1.0
                # dictionary, unchanged in V2.0/V3.x):
                #   1 = Mortgagor Payoff       → voluntary*
                #   2 = Repurchase of Delinquent Loan → involuntary
                #   3 = Foreclosure            → involuntary
                #   4 = Loss Mitigation        → involuntary (distress workout)
                #   5 = Substitution           → administrative pool swap;
                #         falls through to disappearance fallback (rarely
                #         observed; doesn't appear in the current panel)
                #   6 = Other                  → voluntary*
                #
                # *Override (added 2026-05): if a code-1 or code-6 exit
                # has months_dq >= 3 (i.e., is_seriously_dq=1, meaning
                # 90+DPD at exit) we reclassify it as involuntary.
                # Issuers occasionally tag distressed exits as
                # "Mortgagor Payoff" or "Other" when the loan
                # technically pays off (short sale, distressed refi,
                # negotiated payoff) — but for cashflow modeling these
                # are credit events and belong on the involuntary side.
                # The 90+DPD threshold matches GNMA's own definition of
                # seriously-delinquent and the standard industry
                # convention for credit-event triggers. Without this
                # override, ~43 buyout events were misclassified as
                # voluntary prepays. The case that surfaced this:
                # 36179HH87_000000008211099 (HARBORTOWN PROPERTIES,
                # 207/223(f)) exited at mdq=6 with rm=6 and was being
                # called voluntary despite being structurally a
                # delinquent payoff.
                #
                # Code 6 was previously misclassified as involuntary
                # (before the prior fix); empirically, code-6 exits
                # show 99% mdq=0 and 91% paying a prepay penalty —
                # the textbook voluntary-prepay fingerprint. Per
                # GNMA's documentation "Other" is a generic catch-all
                # for non-standard payoff dispositions (sale of the
                # property, refi outside FHA, defeasance, etc.) and
                # carries no inherent delinquency connotation. The
                # case that surfaced THAT issue: pool 3617BQ3M8
                # (CITADEL CARE CENTER-ELGIN, a 232/223(f) healthcare
                # loan) was tagged code 6 with mdq=0 and a 10-point
                # penalty paid at exit — unmistakably a voluntary
                # prepay. So code 6 alone is voluntary; only when
                # ALSO seriously-DQ does it flip to involuntary.
                #
                # NEXT-PERIOD-EVIDENCE GATE (added 2026-05): when no
                # next period is available (the loan is in the most
                # recent period of the panel, or the next period
                # parsed empty), we have no way to confirm the loan
                # actually exits. The prior code path applied vol/invol
                # flags whenever removal_reason was populated, which
                # injected ~75 spurious events in the most recent
                # panel period — issuers occasionally pre-populate
                # removal_reason for upcoming-month events that
                # haven't actually occurred yet, or the field carries
                # over briefly from prior data quality issues. Treating
                # "no next-period evidence" as "cannot determine exit"
                # matches the conservative posture already used by the
                # disappearance-fallback path. Loans truly exiting in
                # the panel's last period will be picked up correctly
                # when the panel is extended forward and that period
                # is no longer the last one.
                has_next_period_evidence = bool(nl)
                seriously_dq_at_exit = (mdq >= 3)
                if not has_next_period_evidence:
                    # No way to confirm exit — treat as still_active
                    vol = False
                    invol = False
                elif rm in ('1', '6'):
                    if seriously_dq_at_exit:
                        # Distressed payoff — override administrative
                        # coding and call it an involuntary buyout.
                        vol = False
                        invol = True
                    else:
                        vol = True
                ...
                if disappeared and not vol and not invol:
                    # Disappearance fallback: in-lockout loans
                    # disappearing with mdq=0 is suspicious (data
                    # quality or unmodeled event); leave both at 0.
                    if in_lockout:
                        if mdq > 0:
                            invol = True
                    else:
            ...
            # ── Exit type (categorical) ────────────────────────────
            if is_construction:
                r['exit_type'] = 'pool_conversion' if disappeared else 'still_active'
            elif r['prepaid_voluntary']:
                r['exit_type'] = 'voluntary_prepay'
            elif r['prepaid_involuntary']:
                r['exit_type'] = 'involuntary_buyout'
            elif disappeared:
                # Disappearance with no clear reason and no flags set
                # — could be near-maturity payoff or unmodeled event.
                mat_yyyymm = parse_date_yyyymmdd(r.get('loan_maturity_date', ''))
                if mat_yyyymm:
                    try:
                        my, mm = int(mat_yyyymm[:4]), int(mat_yyyymm[4:6])
                        py, pm = int(period[:4]), int(period[4:6])
                        months_to_maturity = (my - py) * 12 + (mm - pm)
                        if months_to_maturity <= 3:
                            r['exit_type'] = 'scheduled_maturity'
                        else:
                            r['exit_type'] = 'unknown_exit'
                    except (ValueError, IndexError):
                        r['exit_type'] = 'unknown_exit'
                else:
                    r['exit_type'] = 'unknown_exit'
            else:
                r['exit_type'] = 'still_active'

            period_records.append(r)

        # Convert this period's dicts to a DataFrame immediately and free
        # the raw dicts. Only ~1 period of Python dicts lives in memory
        # at a time; the rest is stored as compact numpy arrays.
        period_dfs.append(pd.DataFrame(period_records))
        del period_records, recs
        gc.collect()

    # Free the sliding-window cache (set if the last period file was empty).
    del recs_cache

    # Concatenate all period DataFrames. DataFrames use numpy arrays, so the
    # combined memory is a small fraction of what all-at-once Python dicts used.
    if not period_dfs:
        return None
    ...
    # ── Post-pass: clean up false-positive exits from panel gaps ────
    # The streaming enrichment loop checks only the IMMEDIATE next
    # period for the disappearance flag. A small number of loans
    # (~24 in a 1.3M-row, 88-month panel) have non-contiguous panel
    # presence — they vanish for 1-2 months due to a GNMA reporting
    # gap and then reappear with the same loan_id. The streaming
    # logic incorrectly flags those gap-edge periods as voluntary
    # prepay or involuntary buyout events.
    #
    # The fix: any row whose exit_type is voluntary_prepay or
    # involuntary_buyout but whose loan_id continues to appear in a
    # period AFTER the flagged one is reset to still_active. The true
    # exit (if any) is correctly captured at the loan's actual final
    # appearance.
    if 'exit_type' in df.columns and len(df) > 0:
        last_period_per_loan = df.groupby('loan_id', observed=True)['period'].transform('max')
        is_loans_actual_last_row = (df['period'] == last_period_per_loan)
        falsely_flagged = (
            df['exit_type'].isin(['voluntary_prepay', 'involuntary_buyout'])
            & ~is_loans_actual_last_row
        )
        n_false = int(falsely_flagged.sum())
        if n_false:
            # exit_type may be category dtype after concat. Cast to
            # object before assignment so 'still_active' assigns even
            # if it's not in the existing categories list.
            if isinstance(df['exit_type'].dtype, pd.CategoricalDtype):
                df['exit_type'] = df['exit_type'].astype('object')
            df.loc[falsely_flagged, 'exit_type'] = 'still_active'
            df.loc[falsely_flagged, 'prepaid_voluntary'] = 0
            df.loc[falsely_flagged, 'prepaid_involuntary'] = 0
            df.loc[falsely_flagged, 'disappeared_next_period'] = 0
    ...
    # Their `prepay_desc` fields explicitly note things like "Prepayment
    # End Date adjusted as per Ginnie Mae's instructions ..." but the
    # numeric date fields were never updated. All affected loans are
    # old enough that both dates are pre-2018, so they're past both
    # lockout and penalty by the time this panel begins (Dec 2018) and
    # in_lockout / in_prepay_penalty correctly read 0 for them. The
    # quirk would matter if the panel were extended further back; for
    # now it's documented and left alone rather than risk breaking
    # cases where the dates are correctly ordered.


    # ── SATO computation (Spread At Origination) ──────────────────
    # SATO = loan_rate − vintage-year median loan_rate, in bps.
    # Loans with above-market SATO accepted higher rates at
    # origination — historically less rate-sensitive borrowers,
    # prepaying slower in refi waves.
    #
    # Two methodological choices worth being explicit about:
    #
    # 1. Filter: we exclude construction-phase rows (CL/CS rates
    #    differ structurally from permanent project loans) and rows
    #    with invalid data, but KEEP loans currently in lockout.
    #    Excluding in-lockout loans (as `prepay_eligible == 1` does)
    #    biases recent vintages because newer loans spend most of
    #    their early life in lockout — the median would then be
    #    computed only on the small subset of recent-vintage loans
    #    that have already exited lockout, which over-represents
    #    early prepays and IRR refis. Loan rates don't depend on
    #    lockout status, so there's no reason to filter on it.
    #
    # 2. Unique-loan dedup: we take each loan's rate once via
    #    groupby('loan_id').first() before computing the vintage
    #    median. The naive approach (median over loan-MONTH
    #    observations) overweights long-lived loans by the number of
    #    monthly snapshots they appear in. Since long-lived loans
    #    skew lower-rate (they didn't prepay because they're at lower
    #    rates), naive medians are biased downward and inflate SATO
    #    for high-rate loans that prepaid early.
    if 'vintage_year' in df.columns and df['vintage_year'].notna().any():
        sato_filter = (
            (df['has_valid_data'] == 1) &
            (df['is_construction_phase'] == 0) &
            df['vintage_year'].notna()
        )
        # One row per loan (its loan_rate doesn't change across
        # snapshots for fixed-rate loans, so .first() is correct).
        unique_loans = (
            df.loc[sato_filter, ['loan_id', 'vintage_year', 'loan_rate']]
              .groupby('loan_id', as_index=False)
              .first()
        )
        vintage_medians = (
            unique_loans.groupby('vintage_year')['loan_rate'].median()
        )
        df['vintage_median_rate'] = df['vintage_year'].map(vintage_medians)
        df['sato_bps'] = ((df['loan_rate'] - df['vintage_median_rate']) * 100).round(2)
    else:
        df['vintage_median_rate'] = np.nan
        df['sato_bps'] = np.nan

    # ── Summary stats ─────────────────────────────────────────────
    total_rows = len(df)
    eligible = df[df['prepay_eligible'] == 1]
    invol_eligible = df[df['involuntary_eligible'] == 1]
    n_construction = (df['is_construction_phase'] == 1).sum()
    n_lockout = ((df['in_lockout'] == 1) & (df['is_construction_phase'] == 0)).sum()
    vol = eligible['prepaid_voluntary'].sum()
    invol_total = df['prepaid_involuntary'].sum()  # includes lockout buyouts
    n_dq30 = (df['is_dq_30'] == 1).sum()
    n_dq90 = (df['is_dq_90'] == 1).sum()

    print(f"\n  ─── Record counts ──────────────────────────────")
    print(f"  Total records:                   {total_rows:>12,}")
    print(f"  Construction phase (CL/CS):      {n_construction:>12,}  (excluded from all prepay/buyout models)")
    print(f"  In lockout (non-construction):   {n_lockout:>12,}  (excluded from voluntary, kept for buyout)")
    print(f"  Prepay-eligible (voluntary):     {len(eligible):>12,}")
    print(f"  Involuntary-eligible (buyout):   {len(invol_eligible):>12,}")
    print(f"\n  ─── Event counts ──────────────────────────────")
    print(f"  Voluntary prepays:               {vol:>12,}")
    print(f"  Involuntary buyouts:             {invol_total:>12,}")
    print(f"  Disappeared (next period):       {df['disappeared_next_period'].sum():>12,}")
    print(f"\n  ─── Exit type breakdown ────────────────────────")
    for et, n in df['exit_type'].value_counts().items():
        print(f"    {et:.<28s} {n:>12,}")
    print(f"\n  ─── Loan purpose breakdown ─────────────────────")
    for lp, n in df['loan_purpose'].value_counts().items():
        print(f"    {lp:.<28s} {n:>12,}")
    print(f"\n  ─── FHA category breakdown ────────────────────")
    for fc, n in df['fha_category'].value_counts().items():
        print(f"    {fc:.<28s} {n:>12,}")
    print(f"\n  ─── Delinquency breakdown ─────────────────────")
    print(f"    30+ DPD: {n_dq30:>10,}    90+ DPD: {n_dq90:>10,}")
    print(f"\n  ─── Numeric ranges ─────────────────────────────")
    if df['refi_incentive_bps'].notna().any():
        print(f"    Refi incentive range:  "
              f"{df['refi_incentive_bps'].min():.0f} to "
              f"{df['refi_incentive_bps'].max():.0f} bps")
    if df['loan_age_months'].notna().any():
        print(f"    Loan age range:        "
              f"{df['loan_age_months'].min():.0f} to "
              f"{df['loan_age_months'].max():.0f} months")
    if df['sato_bps'].notna().any():
        print(f"    SATO range:            "
              f"{df['sato_bps'].min():.0f} to "
              f"{df['sato_bps'].max():.0f} bps")
...
def write_csv(df):
    """Write enriched DataFrame to CSV (gzip) and parquet.

    The CSV.gz output preserves the existing filename pattern that
    downstream tools (ML feature pipeline, dashboard) read. The parquet
    output is a faster-loading alternative — pyarrow can stream the
    1.3M-row file in ~1 second versus ~30 seconds for CSV.gz, which
    matters when iterating on feature engineering.
    """
    df.to_csv(OUTPUT_CSV, index=False)

    # Parquet output (optional — silently skipped if pyarrow missing)
    parquet_path = OUTPUT_CSV.replace('.csv.gz', '.parquet')
    try:
    ...
    if 'period' in df.columns and df['period'].notna().any():
        ...
        period_range = f"{min(periods)} to {max(periods)}"
    else:
        n_periods = 0
        period_range = "unknown"

    print(f"\n{'='*60}")
    print(f"  ok {OUTPUT_CSV} ({os.path.getsize(OUTPUT_CSV)//1024} KB)")
    print(parquet_msg)
    print(f"     {len(df):,} records, {df['loan_id'].nunique():,} unique loans")
    print(f"     {len(df.columns)} columns, {n_periods} periods ({period_range})")
    print(f"{'='*60}")
    return df
...
""")
    ...
    if not file_by_period:
        print(f"No recognizable files in {DATA_DIR}"); return

    df = build_analytics(file_by_period)
